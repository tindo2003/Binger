 ...
 useEffect(() => {
    ...
    fetch(`http://${config.server_host}:${config.server_port}/show/${showName}`)
      ...
      .then(resJson => {
        ...
        fetch(`http://${config.server_host}:${config.server_port}/services/${showName}?type=TV Show`)
          .then(res => res.json())
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2>Director: {showData.director ?? 'N/A'}
        ...
        <p>Duration: {showData.duration} {showData.duration === 1 ? 'Season' : 'Seasons'} </p>
