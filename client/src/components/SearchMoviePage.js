...
function SearchMoviePage() {
  ...
  const search = () => {
    ...
    `&original_title=${title}&releaseYearMax=${releaseYear[1]}&releaseYearMin=${releaseYear[0]}&${params.toString()}`
  ...
  */
  { field: 'original_title', headerName: 'Title', width: 300, renderCell: (params) => (
    <Link onClick={() => {setSelectedMovieName(params.row.original_title); console.log(selectedMovieName); console.log(params.row.budget)}}>{params.value}</Link>
) },
...
<Grid item xs={3}>
  ...
  <FormControl fullWidth>
    ...
    <Select
      ...
      label="Age"
      onChange={(e)=> {setLanguage(codes[e.target.value]);
